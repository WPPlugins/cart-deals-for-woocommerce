...
//DON'T NEED THIS ==> PICKED UP BY 'woocommerce_cart_updated' <==     add_action( 'woocommerce_add_to_cart' ,                   array(&$this, 'vtcrt_ajax_add_to_cart_hook'), 10, 6 );
   //MAYBE++ add_action( 'woocommerce_ajax_added_to_cart' ,              array(&$this, 'vtcrt_ajax_add_to_cart_hook2'), 10, 1 );
    ...
    //v1.0.5 - this filter only works with < woo 3.0.0
    add_filter( 'woocommerce_get_shop_coupon_data',  array(&$this, 'vtcrt_woo_maybe_load_discount_amount_to_coupon'), 5,2); //v1.0.5
    
    //v1.0.5 - this filter only works with > woo 3.0.0
    // UPDATE coupon amount, if 'coupon discount' selected - WOO 3.0.0 and beyond
    add_action( 'woocommerce_coupon_loaded',         array(&$this, 'vtcrt_woo_new_load_discount_amount_to_coupon'), 10,1);
...
//************************************************* 
    ...
    add_action( 'woocommerce_before_mini_cart', array(&$this, 'vtcrt_maybe_recalc_woo_totals'), 10, 1 );  
    ...
    */
      ...
      if ( ($vtcrt_cart->lifetime_limit_applies_to_cart == 'yes') && ( sizeof($vtcrt_cart->error_messages) == 0 ) ) {   //error msg > 0 = 2nd time through HERE, customer has blessed the reduction
        ...
        *************************************************   */
        ...
        $data_chain[] = vtcrt_get_current_user_role();  //v1.0.3
  ...
  *************************************************** */
   ...
   */
    ...
    */
    ...
    if ($vtcrt_info['product_session_info']['product_yousave_total_amt'] > 0)  {     //v1.0.3  replaced 'product_discount_price' with 'product_yousave_total_amt' to pick up a FREE discount
...
*/ 
    if ($vtcrt_info['product_session_info']['product_yousave_total_amt'] > 0)  {     //v1.0.7.2  replaced 'product_discount_price' with 'product_yousave_total_amt' to pick up a FREE discount
      $price = $vtcrt_info['product_session_info']['product_discount_price'];
    }     
...
*/  
    ...
    // we could use $product_id  =  $product_info->get_id(); , but really don't nned to - this is a CART ONLY, auto-inserted coupon plugin, after all!
...
*/    
    ...
    if ( ($woocommerce->cart->cart_contents_total  > 0) &&   //V1.0.3  if == 0, lost addressability to woo, rerun
         ...
         ($previous_user_role                      ==  vtcrt_get_current_user_role() ) )  { //v1.0.3  only return if user_role has not changed          
       return;  
    }
...
*/
  ...
  //clears coupon from cart
   public function vtcrt_woo_maybe_remove_coupon_from_cart($coupon_title) {
      ...
      WC()->cart->remove_coupon( $coupon_title );   //v1.0.3 
      
      /* //v1.0.3  'if' replaced with the above
   ...
   public function vtcrt_woo_maybe_load_discount_amount_to_coupon($status, $code) {
      ...
      // if woo 3.0.0 or newer, handled in function vtcrt_woo_new_load_discount_amount_to_coupon
      if ( version_compare( WC_VERSION, '3.0.0', '>=' ) ) { //check if o>= than version 3.0.0  
        return false;
      }
      //v1.0.5 end
  ...
  //echo '$vtcrt_rules_set= <pre>'.print_r($vtcrt_rules_set, true).'</pre>' ;
   ...
   // UPDATE coupon amount, if 'coupon discount' selected - WOO 3.0.0 and beyond
   //****************************************************************
	public function vtcrt_woo_new_load_discount_amount_to_coupon($coupon) {
  ...
  //DO NOT RUN IN ADMIN!!!!!!!! (this function is called everywhere)
    ...
    global $woocommerce, $vtcrt_cart, $vtcrt_info, $vtcrt_rules_set, $vtcrt_setup_options;
     
      //error_log( print_r(  'vtcrt_woo_new_load_discount_amount_to_coupon BEGIN, coupon= ', true ) );
      //error_log( var_export($coupon, true ) );
    
    //before woo 3.0.0, handled in function vtcrt_woo_maybe_load_discount_amount_to_coupon
...
/*  
    ...
    if ($vtcrt_rules_set == '') { 
        error_log( print_r(  'vtcrt_rules_set is blank ', true ) );    
    }
 */   
     
   
      //error_log( print_r(  "code = " .$coupon->get_code() , true ) );
      //error_log( print_r(  "coupon_code_discount_deal_title = " .$vtcrt_info['coupon_code_discount_deal_title'] , true ) );
      //error_log( print_r(  "amount = " .$coupon->get_amount() , true ) );
      //error_log( print_r(  "vtcrt_cart->yousave_cart_total_amt = " .$vtcrt_cart->yousave_cart_total_amt , true ) );      
    
    if ( ($coupon->get_code() != $vtcrt_info['coupon_code_discount_deal_title']) ||
         ($coupon->get_amount() == $vtcrt_cart->yousave_cart_total_amt) ) {
       return $coupon;
    }    
...
/* ************************************************
 ...
 //Clean Up Session Variables which would otherwise persist during Discount Processing       
  public function vtcrt_maybe_clear_auto_add_session_vars() {
   ...
   public function vtcrt_maybe_recalc_woo_totals() {
     global $woocommerce;
     $woocommerce->cart->calculate_totals();        
     return;
   }
