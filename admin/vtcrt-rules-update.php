...
*/
 ...
 /*     $vtcrt_rule->ruleApplicationPriority_num = preg_replace('/[^0-9.]+/', '', $vtcrt_rule->ruleApplicationPriority_num); //remove leading/trailing spaces, percent sign, dollar sign
  ...
  public function vtcrt_update_rules_info() {
    global $post, $vtcrt_rule, $vtcrt_rules_set, $vtcrt_setup_options; 

    //v1.0.3
    if ( $vtcrt_setup_options['debugging_mode_on'] == 'yes' ){   
      error_log( print_r(  '$vtcrt_rule', true ) );
      error_log( var_export($vtcrt_rule, true ) );   
    }
/*      
  ...
  *************************************************** */
  public function vtcrt_get_variations_parameter($which_vargroup) {
    ...
    //************************
	  ...
	  $adding_to_cart      = wc_get_product( $product_id ); //v1.0.5 replace get_product with wc_get_product

  	$all_variations_set = true;
  	$variations         = array();

		$attributes = $adding_to_cart->get_attributes();
		$variation  = wc_get_product( $variation_id ); //v1.0.5 replace get_product with wc_get_product
